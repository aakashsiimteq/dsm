...
$shape_id = "";
  if (isset($_POST['shape'])) {
    ...
    $diamond  = "SELECT * FROM `diamonds` WHERE diamond_shape_id = '$shape_id'";
    $sql = mysqli_query($conn, $diamond);
  }else {
    $diamond  = "SELECT * FROM `diamonds` WHERE diamond_shape_id = 2 AND `diamond_size` >= 0.90";
    $sql = mysqli_query($conn, $diamond);
  }
  $shape = "SELECT distinct(`attribute_name`),`attribute_id` FROM `attributes` where `attribute_type` = 'Shape'";
    ...
    <form method="POST" action="">
      ...
      <fieldset class="form-group">
          <?php while($row_shape = mysqli_fetch_assoc($sql2)): ?>
            <?php
                 $sql_count = mysqli_query($conn, "SELECT COUNT(*) from `diamonds` WHERE `diamond_shape_id` = '".$row_shape['attribute_id']."'");
                 $row_count = mysqli_fetch_array($sql_count);
                ?>
            <button value="<?=$row_shape['attribute_id']?>" class="bg-primary" name = 'shape' type="submit"><?= $row_shape['attribute_name']."(".$row_count[0].")" ?></button>
          <?php endwhile;?>
      </fieldset>
